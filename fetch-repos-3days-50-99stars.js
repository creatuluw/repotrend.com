...
// Main function
async function main() {
  const date3daysAgo = getDate3DaysAgo();
  const dateString = getDateString();

  // Ensure data directory exists
  const dataDir = path.join(__dirname, "data", "3days-50-99stars");
  if (!fs.existsSync(dataDir)) {
    fs.mkdirSync(dataDir, { recursive: true });
  }

  const filename = `repos-3days-50-99stars-${dateString}.json`;
  const filepath = path.join(dataDir, filename);

  // Build API URL: repos with 50-99 stars (using range syntax)
  const query = encodeURIComponent(`created:>=${date3daysAgo} stars:50..99`);
  const apiUrl = `https://api.github.com/search/repositories?q=${query}&sort=stars&order=desc&per_page=100`;

  console.log(
    `Fetching repos created since ${date3daysAgo} with 50-99 stars...`,
  );
