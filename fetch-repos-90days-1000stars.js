...
// Main function
async function main() {
  const date90daysAgo = getDate90DaysAgo();
  const dateString = getDateString();

  // Ensure data subdirectory exists
  const dataDir = path.join(__dirname, "data", "90days-1000stars");
  if (!fs.existsSync(dataDir)) {
    fs.mkdirSync(dataDir, { recursive: true });
  }

  const filename = `repos-last90days-1000stars-${dateString}.json`;
  const filepath = path.join(dataDir, filename);

  // Build API URL
  const query = encodeURIComponent(`created:>=${date90daysAgo} stars:>=1000`);
  const apiUrl = `https://api.github.com/search/repositories?q=${query}&sort=stars&order=desc&per_page=100`;

  console.log(
    `Fetching repos created since ${date90daysAgo} with >= 1000 stars...`,
  );
